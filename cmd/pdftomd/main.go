...
func main() {
	...
	lastPage := flag.Int("l", 0, "last page to convert (0 = last page)")
	includeImages := flag.Bool("images", false, "include image references in markdown")
	extractImages := flag.Bool("extract-images", false, "extract images to files")
	imagePrefix := flag.String("image-prefix", "image", "prefix for extracted image files")
	pageSeparator := flag.String("separator", "\n\n---\n\n", "page separator in output")
	headingDetection := flag.Bool("headings", true, "enable automatic heading detection")
	...
	if info.Encrypted {
		...
		if password != "" {
			...
			fmt.Fprintf(os.Stderr, "Error: PDF is encrypted, password required\n")
			os.Exit(1)
		}
	}
